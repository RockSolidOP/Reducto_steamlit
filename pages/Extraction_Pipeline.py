...
def _build_plan_1040(classified: List[ClassifiedPage]) -> Tuple[List[AzureJob], List[PagePlan]]:
    ...
    # Separate 1040 main pages and Schedule 1 / Schedule A / Schedule C from others
    ...
    # 1040 Schedule C: create single-page jobs using schedule C model
    if schc_pages:
        model_id_schc = AZURE_CONFIG.get("model_id_1040_schedule_c", "prebuilt-tax.us.1040ScheduleC")
        for p in sorted(schc_pages):
            pair_number += 1
            group_id = f"{file_name}#1040_SCHC#{pair_number}({p})"
            job_id = group_id
            azure_jobs.append(
                AzureJob(job_id=job_id, file_name=file_name, model_id=model_id_schc, pages=str(p), reason="1040 Schedule C"),
            )
            page_plan.append(
                PagePlan(page=p, label="1040_Schedule_C", status="scheduleC", pair_number=pair_number, group_id=group_id, job_id=job_id, action="analyze", model_id=model_id_schc)
            )
    ...
    *,
    ...
    for pp in page_plan:
        ...
        if pp.action == "analyze" and azure_model:
            if pp.status in ("paired", "schedule1", "scheduleA", "scheduleC"):
...
def run() -> None:
    ...
    job_policy = {
        "rules": [
            ...
            {
                "if_family": "1040",
                "labels": ["1040_Schedule_C"],
                "model": AZURE_CONFIG.get("model_id_1040_schedule_c", "prebuilt-tax.us.1040ScheduleC"),
            },
        ]
    }
