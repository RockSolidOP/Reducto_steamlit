...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Azure DI prebuilt Schedule A model")
    parser.add_argument(
        "--file",
        default=str(Path(__file__).resolve().parents[2] / "testing_files" / "sample_multiple.pdf"),
        help="Path to PDF/image containing Schedule A",
    )
    parser.add_argument(
        "--pages",
        default="21",  # Schedule A is page 21 in the sample file
        help="Optional comma-separated pages to analyze (e.g., '1' or '2,3'). Default: 21",
    )
    parser.add_argument(
        "--model-id",
        default="prebuilt-tax.us.1040ScheduleA",
        help="Azure model ID to use (default: prebuilt-tax.us.1040ScheduleA)",
    )
    parser.add_argument(
        "--out",
        default=None,
